...
public:
    ...
    const std::vector<deepsort_track_310p::TensorDesc> &inputDescs() const { return input_descs_; }
    const std::vector<deepsort_track_310p::TensorDesc> &outputDescs() const { return output_descs_; }
    bool prepareStage(std::string *error);
    bool copyInputsToStage(const std::vector<deepsort_track_310p::TensorData> &inputs,
                           std::string *error);
    bool executeStage(std::string *error);
    bool copyOutputsFromStage(std::vector<deepsort_track_310p::TensorData> &outputs,
                              std::string *error);
...
private:
    ...
    std::vector<deepsort_track_310p::TensorDesc> input_descs_;
    std::vector<deepsort_track_310p::TensorDesc> output_descs_;
    deepsort_track_310p::DeviceExecutionStage stage_;
};
