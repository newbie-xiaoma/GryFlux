...
void setReidErrorText(std::string *error, const std::string &message)
{
    if (error != nullptr) {
        *error = message;
    }
...
std::string reidPrefixFor(bool isInput)
{
    return isInput ? "input" : "output";
}

bool sameReidTensorDescs(const std::vector<deepsort_track_310p::ReidTensorDesc> &lhs,
                         const std::vector<deepsort_track_310p::ReidTensorDesc> &rhs)
{
...
ReidDeviceExecutionStage::~ReidDeviceExecutionStage()
{
    release();
}

bool ReidDeviceExecutionStage::prepare(aclrtContext context,
                                       ...
                                       std::string *error)
{
    if (ready_ &&
        context_ == context &&
        sameReidTensorDescs(inputDescs_, inputDescs) &&
        sameReidTensorDescs(outputDescs_, outputDescs)) {
        return true;
    }
...
bool ReidDeviceExecutionStage::buildDatasets(std::string *error)
{
    if (context_ == nullptr) {
...
void ReidDeviceExecutionStage::release()
{
...
void ReidDeviceExecutionStage::setError(std::string *error, const std::string &message)
{
    if (error != nullptr) {
        *error = message;
    }
...
size_t ReidTensorData::elementCount() const
{
    if (dims.empty()) {
        return 0;
    }
                           ...
                           std::string *error)
{
    if (tensor.dtype != desc.dtype) {
        setReidErrorText(error,
                         "tensor dtype mismatch, expect=" + reidAclDataTypeName(desc.dtype) +
                             ", got=" + reidAclDataTypeName(tensor.dtype));
        return false;
    }

    if (tensor.dims != desc.dims) {
        setReidErrorText(error,
                         "tensor dims mismatch, expect=" + reidDimsToString(desc.dims) +
                             ", got=" + reidDimsToString(tensor.dims));
        return false;
    }
    ...
    , device_id_(device_id)
{
    ...
    if (input_descs_.size() != 1 || output_descs_.size() != 1) {
        throw std::runtime_error("deepsort_track_310p reid expects exactly 1 input and 1 output");
    }
}
...
bool ReidContext::init(std::string *error)
{
    if (initialized_) {
        return true;
    }
    ...
    if (!environment_acquired_) {
        if (!deepsort_track_310p::AclEnvironment::acquire(device_id_, error)) {
            return false;
        }
...
void ReidContext::destroy()
{
    ...
    if (environment_acquired_) {
        deepsort_track_310p::AclEnvironment::release(device_id_);
        environment_acquired_ = false;
    }
}

bool ReidContext::prepareStage(std::string *error)
{
...
bool ReidContext::copyInputsToStage(const std::vector<deepsort_track_310p::ReidTensorData> &inputs,
                                    std::string *error)
{
    ...
    for (size_t i = 0; i < inputs.size(); ++i) {
        if (!deepsort_track_310p::reidTensorMatchesDesc(inputs[i], input_descs_[i], error)) {
...
bool ReidContext::executeStage(std::string *error)
{
    const aclError setRet = aclrtSetCurrentContext(context_);
    if (setRet != ACL_ERROR_NONE) {
        setError(error, "aclrtSetCurrentContext failed, code=" + std::to_string(setRet));
...
bool ReidContext::copyOutputsFromStage(std::vector<deepsort_track_310p::ReidTensorData> &outputs,
                                       std::string *error)
{
    const aclError setRet = aclrtSetCurrentContext(context_);
    if (setRet != ACL_ERROR_NONE) {
...
bool ReidContext::queryTensorDesc(bool isInput,
                                  size_t index,
                                  ...
                                  std::string *error)
{
    ...
    if (dimRet != ACL_ERROR_NONE) {
        setError(error,
                 reidPrefixFor(isInput) + "[" + std::to_string(index) +
...
void ReidContext::setError(std::string *error, const std::string &message)
{
    if (error != nullptr) {
        *error = message;
    }
