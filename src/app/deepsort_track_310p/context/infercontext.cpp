...
bool sameTensorDescs(const std::vector<deepsort_track_310p::TensorDesc> &lhs,
                     const std::vector<deepsort_track_310p::TensorDesc> &rhs)
{
    ...
    , device_id_(device_id)
{
    ...
    if (input_descs_.size() != 1) {
        throw std::runtime_error("deepsort_track_310p detector expects exactly 1 model input");
    }
}
...
bool InferContext::copyInputsToStage(const std::vector<deepsort_track_310p::TensorData> &inputs,
                                     std::string *error)
{
    ...
    for (size_t i = 0; i < inputs.size(); ++i) {
        if (!deepsort_track_310p::tensorMatchesDesc(inputs[i], input_descs_[i], error)) {
...
bool InferContext::copyOutputsFromStage(std::vector<deepsort_track_310p::TensorData> &outputs,
                                        std::string *error)
{
    const aclError setRet = aclrtSetCurrentContext(context_);
    if (setRet != ACL_ERROR_NONE) {
...
bool InferContext::queryTensorDesc(bool isInput,
                                   size_t index,
                                   deepsort_track_310p::TensorDesc &desc,
                                   std::string *error)
{
    desc.index = index;
