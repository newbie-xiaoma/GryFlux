...
public:
    ...
    const std::vector<yolox_detection_310p::TensorDesc> &inputDescs() const { return input_descs_; }
    const std::vector<yolox_detection_310p::TensorDesc> &outputDescs() const { return output_descs_; }
    bool prepareStage(std::string *error);
    bool copyInputsToStage(const std::vector<yolox_detection_310p::TensorData> &inputs,
                           std::string *error);
    bool executeStage(std::string *error);
    bool copyOutputsFromStage(std::vector<yolox_detection_310p::TensorData> &outputs,
                              std::string *error);
...
private:
    ...
    std::vector<yolox_detection_310p::TensorDesc> input_descs_;
    std::vector<yolox_detection_310p::TensorDesc> output_descs_;
    yolox_detection_310p::DeviceExecutionStage stage_;
};
