...
bool sameTensorDescs(const std::vector<image_fusion_310p::TensorDesc> &lhs,
                     const std::vector<image_fusion_310p::TensorDesc> &rhs)
{
...
FusionModelInfo BuildModelInfo(const InferContext &context, int width, int height)
{
    ...
    model_info.input_element_count = context.inputDescs().at(0).bytes / sizeof(float);
    model_info.output_element_count = context.outputDescs().at(0).bytes / sizeof(float);
    return model_info;
}
    ...
    , device_id_(device_id)
{
    ...
    if (input_descs_.size() != 2 || output_descs_.size() != 1) {
...
bool InferContext::init(std::string *error)
{
    ...
    if (!environment_acquired_) {
        if (!image_fusion_310p::AclEnvironment::acquire(device_id_, error)) {
            return false;
        }
...
void InferContext::destroy()
{
    ...
    if (environment_acquired_) {
        image_fusion_310p::AclEnvironment::release(device_id_);
...
bool InferContext::copyInputsToStage(const std::vector<image_fusion_310p::TensorData> &inputs,
                                     std::string *error)
{
    ...
    for (size_t i = 0; i < inputs.size(); ++i) {
        if (!image_fusion_310p::tensorMatchesDesc(inputs[i], input_descs_[i], error)) {
...
bool InferContext::copyOutputsFromStage(std::vector<image_fusion_310p::TensorData> &outputs,
                                        std::string *error)
{
    const aclError setRet = aclrtSetCurrentContext(context_);
    if (setRet != ACL_ERROR_NONE) {
